...
# print(np.mean(conf_mat[part], axis = 0))
with open(str(Path.cwd().parent / 'results_used' / 'indiv_classification_tasks_cf_force_morefeat.pkl'), 'wb') as handle:
    pickle.dump(conf_mat, handle, protocol=pickle.HIGHEST_PROTOCOL) 

with open(str(Path.cwd().parent / 'results_used' / 'indiv_classification_tasks_report_force_morefeat.pkl'), 'wb') as handle:
