...
df.drop(['dev', 'var'], axis = 1, inplace = True)
parts = pd.unique(df.part)
param_grid = { 'svc__C': [0.1, 1, 10, 100, 1000, 1500, 2000, 10000],
...
for part in parts: 
    print('processing participant %s' %part) 
    ...
    X = df_part.drop(['trial','label','part'], axis = 1).values   
    results_per_fold = []
    confmat_per_fold = []
...
with open(str(Path.cwd().parent / 'results' / 'task_classification' /'classification_report_force_other_features_cf.pkl'), 'wb') as handle:
    pickle.dump(conf_mat, handle, protocol=pickle.HIGHEST_PROTOCOL) 

with open(str(Path.cwd().parent / 'results' / 'task_classification' / 'classification_report_force_other_features.pkl'), 'wb') as handle:
