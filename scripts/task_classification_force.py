...
with open(str(Path.cwd().parent / 'results' / 'indiv_classification_tasks_cf_force.pkl'), 'wb') as handle:
    pickle.dump(conf_mat, handle, protocol=pickle.HIGHEST_PROTOCOL) 

with open(str(Path.cwd().parent / 'results' / 'indiv_classification_tasks_report_force.pkl'), 'wb') as handle:
